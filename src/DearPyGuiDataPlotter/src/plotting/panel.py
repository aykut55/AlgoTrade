...
class Panel:
    ...
    def __init__(self, panelId: int, name: str, caption: str = "", parent: str = "",
        ...
        self.yLabel = None  # None = Y ekseni etiketi icin caption kullanilir (bkz. setYLabel)
        self.parent = parent
    ...
    def addData(self, dataId: int, name: str = "", dataType: str = "line",
        ...
        color verilirse ("line" tipi seriler icin) cizimde sabit bir renk
        teması uygulanir (bkz. PanelManager._drawOrUpdateSeries), verilmezse
        DPG'nin otomatik atadigi varsayilan renk kullanilir.
        """
        data = PanelData(dataId, name, dataType, xs, ys, color=color)
        data.setParent(self)
    ...
    def setYLabel(self, label: str):
        """Y ekseninde gosterilecek metni caption'dan BAGIMSIZ olarak set eder
        (orn. caption='Signals (AL/SAT/FLAT)' ama Y ekseni sadece 'Signals').
        None verilirse (varsayilan) Y ekseni caption'i kullanir."""
        self.yLabel = label
    ...
    def getYLabel(self): return self.yLabel if self.yLabel is not None else self.caption
