...
from .runtimeCommandManager import RuntimeCommandManager
...
class GuiManager:
    ...
    def __init__(self, configManager):
        ...
        self.runtimeCommandManager = RuntimeCommandManager(self)
        ...
        self._shutdownRequested = False  # bkz. requestShutdown()
    ...
    def requestShutdown(self):
        """RuntimeCommandManager'in 'shutdown' komutu bunu cagirir.
        dpg.is_dearpygui_running(), dpg.stop_dearpygui()'den SONRAKI frame'de
        bile hala True donebiliyor (DPG'nin ic durum guncellemesi bir frame
        gecikmeli) - bu yuzden render()'in split_frame() gerektiren islemleri
        (auto-sync X/Y) bir sonraki tick'te calistırmaya devam edip "no active
        rendering loop" hatasi atmasini onlemek icin KENDI flag'imizi kullaniyoruz."""
    ...
    def render(self):
        ...
        if not dpg.is_dearpygui_running() or self._shutdownRequested:
            return
        ...
        # EN SONDA cagrilir: load_bundle/shutdown gibi komutlar mevcut frame'in
        # geri kalanini (ozellikle _updateAutoSyncX -> dpg.split_frame()) bozmasin
        # diye - shutdown komutu dpg.stop_dearpygui() cagirinca, ayni frame'de
        # SONRASINDA calisan split_frame() "no active rendering loop" hatasi
        # atiyordu (loop zaten durdurulmaya baslanmisti).
        self.runtimeCommandManager.processPendingCommands()
    ...
    def _applyTopPanelInitialState(self):
        ...
        self._onShowPlotCaptionsChanged(appData=cfg["showPlotCaptions"])
    ...
    def _onShowPlotCaptionsChanged(self, sender=None, appData=None):
        """Tum panellerin plot basligini (dpg.add_plot label'i) acar/kapatir.
        Baslik kapatilinca ImPlot o satiri TAMAMEN kaldirir (sadece metni
        boslatmiyor) - PanelManager.setShowCaptions bunu hem su an cizili
        panellere hem BUNDAN SONRA olusturulacak panellere (yeni bundle
        yuklendiginde vb.) uygular (bkz. panelManager.py)."""
        self.panelManager.setShowCaptions(bool(appData))
        ...
        with dpg.child_window(tag="topPanel", parent=self.LAYOUT_ROOT,
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.child_window(tag="topPanelGroupBox4", width=300, height=-1, border=True):
                    ...
                    dpg.add_checkbox(label="Show Plot Captions", tag="top_show_captions_checkbox",
                                     default_value=bool(topCfg["showPlotCaptions"]),
                                     callback=self._onShowPlotCaptionsChanged)
