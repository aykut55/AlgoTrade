...
class TradeSignalRenderer:
    ...
    MAX_SIGNAL_EVENTS = 50000  # 20000 -> 50000: 928K bar'lik gercek veri setinde ~26013 sinyal olayi cikiyor, eski esik harf/cizgileri atliyordu
    ...
    def _drawColoredRuns(self, panelId, dataId, data, runs, yTag):
        """Her run (ayni durumda kalan ARDISIK bar araligi) KENDI candle_series'i
        olarak cizilir - AYNI state'in birbirinden UZAK/kesikli run'larini TEK
        bir seri icinde birlestirmek (onceki 'grouped-by-state' yaklasimi)
        candle genisligi/spacing hesabini bozup 'bulut' gibi gorunen bir
        render artifact'ina yol aciyordu (ozellikle SIK kisa sureli AL/SAT
        gecisi olan veri setlerinde - kullanici bunu bildirdi: SAT run'lari
        az/uzun oldugu icin duzgun, AL run'lari cok/kisa oldugu icin bulutumsu
        gorunuyordu). Per-run cizim, her serinin kendi ICINDE bar'larin
        ARDISIK/duzgun aralikli olmasini garanti eder."""
