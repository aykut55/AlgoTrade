...
class App:
    ...
    def _createPanelFromSpec(self, panelSpec):
        ...
        if panelSpec.get("yLabel"):
            panel.setYLabel(str(panelSpec.get("yLabel")))
        pm.addPanel(panel)
        ...
        yFixedRange = panelSpec.get("yFixedRange")
        if yFixedRange:
            panel.yFixedRange = tuple(yFixedRange)
            panel.ySyncMode = "fixedRange"  # bkz. PanelManager._getFixedYRange
        return panel
            ...
            for seriesSpec in panelSpec.get("series", []):
                ...
                if source == "indicator":
                    ...
                    panel.addData(dataId, label, "line", self.data.xs, ys,
                                  timestamps=self.data.timestamps, intraday=intraday, color=color)
                    continue
